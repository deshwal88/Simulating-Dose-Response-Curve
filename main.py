...
def main():
    ...
    points_c40 = functions.calculate_even_dilution(0.4*top_concentration, dilution_factor)
    c_40 = np.linspace(points_c40[0], points_c40[-1], 50)
    points_c100 = functions.calculate_even_dilution(top_concentration, dilution_factor)
    c_100 = np.linspace(points_c100[0], points_c100[-1], 50)
    points_c160 = functions.calculate_even_dilution(1.6*top_concentration, dilution_factor)
    c_160 = np.linspace(points_c160[0], points_c160[-1], 50)

    #generating response curves for 40, 100 and 160 uM top concentrations
    res_40 = functions.compute_4pl(0.5, 1.5, 2, 3.5, c_40)
    ...
    for factor, series in reco:
        print(f"{factor}-fold → {series}")
